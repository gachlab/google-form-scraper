import type { GoogleFormInput, FormResponse, FormField, GoogleFormsScraperFactory } from './definitions';
import { parse } from 'node-html-parser';

function getForm(dependencies: { fetch: typeof globalThis.fetch; htmlParser: typeof parse }) {
  return async (input: GoogleFormInput): Promise<FormResponse> => {
    ...
    const fields: FormField[] = html
      ...
      .map((item, index) => {
        ...
        const radioSpans = item.querySelectorAll("div[role='radiogroup'] span[dir='auto']");
        const radioLabels = item.querySelectorAll("div[role='radiogroup'] label");
        const listItems = item.querySelectorAll("div[role='list'] div[role='listitem']");
        ...
        const field: FormField = {
          ...
          type: radioSpans.length > 0 ? 'radiogroup'
            : item.querySelector("div[role='radiogroup']") ? 'presentation'
        ...
        if (listItems.length > 0) {
          field.options = listItems.map(li => ({
            ...
            ...(li.querySelector("input[type='text']") ? { isTextField: true } : {}),
          }));
        } else if (radioSpans.length > 0) {
          field.options = radioSpans.map(span => ({ prompt: span.innerText }));
        } else if (radioLabels.length > 0) {
          field.options = radioLabels.map(label => ({
            prompt: label.innerText,
            ...(label.parentNode.querySelector("input[type='text']") ? { isTextField: true } : {}),
          }));
    ...
    return {
      ...
      description: html.querySelector("div[dir='auto']:nth-child(2)")?.innerText,
      fields,
    };
  };
}
